...
void initSPIx()
{
    ...
    if (SPIx == SPI1)
    {
        GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4; //配置PA4为NSS选通引脚
        GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
        GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
        GPIO_Init(GPIOA, &GPIO_InitStructure);
        GPIO_SetBits(GPIOA, GPIO_Pin_4); //默认拉高

        GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7 | GPIO_Pin_5; //配置MOSI(PA7)和SCK(PA5)
        GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
        GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
        GPIO_Init(GPIOA, &GPIO_InitStructure);

        GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6; //配置MISO(PA6)
